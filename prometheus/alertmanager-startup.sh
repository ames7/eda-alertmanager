...
export WEBHOOK_UUID=`curl -X POST -H "Content-Type: application/json" -d '{"default_status": 200, "default_content": "Hello world!", "default_content_type": "application/json", "timeout": 0, "cors": false, "expiry": true, "alias": "webhook", "actions": true}' http://ec2-3-129-183-224.us-east-2.compute.amazonaws.com:8084/token | jq -r '.uuid'`
